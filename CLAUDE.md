...
- **Email** — built-in support channel (`app/Modules/Email/`), same tier as Telegram/VK/Max/Avito; free, ships with core. No HTTP webhook — incoming mail is polled via IMAP (`php artisan email:poll`, scheduled every minute), outgoing replies go over SMTP (`SendEmailMessageJob`). Unlike Avito, Email is not text-only in either direction: it carries **a single file attachment per message**, both incoming (customer's email → forwarded as a Telegram photo/document) and outgoing (manager's reply — from the admin panel OR typed straight into the Telegram group topic — → SMTP attachment). Works with any provider (Yandex, Mail.ru, Gmail with an app-password, a corporate mailbox); no per-provider presets or OAuth in v1 (see `rules/domain/messaging.md`)
...
│       ├── EmailIntegrationPage.php     # /admin/settings/email (built-in Email module IMAP/SMTP credentials)
...
│           ├── email-integration-page.blade.php      # View for EmailIntegrationPage
...
- **Avito v1 is text-only.** Incoming attachments are not accepted; a file attached to a manager's reply is skipped with a warning log instead of being sent (see `SendReplyAction::sendAvitoReply()`). Avito Messenger has no inline-keyboard support, so instead of the 1-5 tappable rating other platforms get, `SendFeedbackForm`'s `case 'avito'` asks the user for a free-text review — a deliberate product choice, not a stopgap. There is nothing to capture back (`HandleFeedbackRating` is never invoked for Avito), so the `Feedback` record stays in `status='awaiting_rating'` forever; any review the user sends just shows up as a normal message in the topic
- **Email carries a single file attachment per message, both ways.** Incoming: `EmailImapClient::extractAttachments()` parses the email's first attachment (only one — Telegram sends one photo/document per message), writing a queue-safe temp copy (fed to Telegram's upload) and a permanent `chat-attachments/` copy (for the admin workspace); `EmailMessageService::sendMessage()` forwards it as `sendPhoto`/`sendDocument` instead of `sendMessage`. Outgoing: `SendReplyAction::sendEmailReply()` (admin panel) and `TgEmailMessageService::sendFileReply()` (typed straight into the Telegram group topic — downloads the manager's Telegram file and re-uploads it as the SMTP attachment) both copy the file to a queue-safe path, record it locally via a `chat-attachments/` `MessageAttachment` (email has no provider file id to fall back on, unlike Telegram), and `EmailMailer::send()` attaches it to the outgoing SMTP message via `Message::attach()`. A file-only reply (no text) still sends on every path.
...
- Email is configured on its own dedicated screen, `/admin/settings/email` (`EmailIntegrationPage`, route name `admin.settings.email`) — NOT through `IntegrationChannelPage` (its generic IMAP/SMTP host/port/encryption fields don't fit the `telegram|telegram_ai|vk|max` shape). Fields: `email.imap_host`, `email.imap_port` (default 993), `email.imap_encryption` (default `ssl`), `email.smtp_host`, `email.smtp_port` (default 587), `email.smtp_encryption` (default `tls`), `email.username`, `email.password`(secret), `email.from_address`, `email.from_name`, `email.poll_interval` (informational in v1 — the `email:poll` scheduler entry runs a fixed `everyMinute()`, not read from this key), `email.ignored_addresses` (JSON array, one entry per line in the UI — a full address or a `@domain.com` suffix to block a whole domain; matched by `EmailIgnoreListMatcher` in `PollInboxCommand` before a `BotUser` is ever created for the sender, so newsletter/no-reply senders never spawn a support topic). The «Сохранить» button runs a verify-before-save flow (`EmailVerificationService::verify()`: a real IMAP login via webklex/php-imap AND a real SMTP handshake via a Symfony Mailer transport's `start()`/`stop()` — both must succeed); nothing is persisted if either check fails. The ignore list is not part of that verification — it is parsed/deduped and persisted alongside the other fields on success. Admin-only; route is always registered since Email ships with core
- Telegram channel page (`channel=telegram`) covers: `telegram.token`(secret), `telegram.secret_key`(secret) — **both required** (`IntegrationChannelPage::validateFields()` blocks «Сохранить»/`connect()` with per-field errors until each is filled; fields are pre-filled from settings, so an existing config already satisfies this). `telegram.group_id` and `telegram.template_topic_name` moved to the General settings screen; the `telegram.bot_id` setting was removed (unused at runtime). A third, **optional** field, `telegram.webhook_ip_address` (plain text, not a secret), is validated as an IP when non-blank and always saved as entered (including blank, to allow clearing it) — when set, `WebhookRegistrationService::registerTelegram()` passes it as `ip_address` to Telegram's `setWebhook`, so Telegram connects to that fixed IP instead of re-resolving `APP_URL`'s domain via DNS on every update
...
- **Avito v1 has no inline-keyboard support**, so `SendFeedbackForm`'s `case 'avito'` branch asks for a free-text review instead of a 1-5 rating — there is no tappable star widget, and this is a deliberate choice rather than a workaround. No rating callback ever arrives: `HandleFeedbackRating` is never invoked for Avito, and the `Feedback` record stays in `status='awaiting_rating'` indefinitely; a review the user sends back is just a normal inbound message in the topic, not captured on the `Feedback` row
