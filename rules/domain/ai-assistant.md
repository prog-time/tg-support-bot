...
**BR-017** — AI runs across all supported user platforms (`telegram`, `vk`, `max`, `avito`, `email`). The trigger for incoming user messages is platform-specific (TG: `TelegramBotController::maybeDispatchAi()`; VK: `VkMessageService::maybeDispatchAi()`; Max: `MaxMessageService::maybeDispatchAi()`; Avito: `AvitoMessageService::maybeDispatchAi()`; Email: `EmailMessageService::maybeDispatchAi()`, called from the polled `email:poll` pipeline instead of a webhook), but the gating logic shares `ShouldAiReply` (both the TG-DTO variant `shouldGenerateForUserMessage()` and the platform-agnostic variant `shouldGenerateForBotUserText()` enforce: AI-enabled, private-chat source, replyable text, user active — no manager-interface check). Triggering is text-only — attachments do not start AI (this also means Avito and Email, both text-only end-to-end in v1, never have an attachment-triggering edge case).
_Enforced in:_ `app/Modules/Ai/Services/ShouldAiReply.php`, `app/Modules/Telegram/Controllers/TelegramBotController.php`, `app/Modules/Vk/Services/VkMessageService.php`, `app/Modules/Max/Services/MaxMessageService.php`, `app/Modules/Avito/Services/AvitoMessageService.php`, `app/Modules/Email/Services/EmailMessageService.php`

**BR-018** — Final delivery of an AI answer to the end user (both after manager Accept and in auto-reply mode) is routed by `BotUser.platform` through `App\Modules\Ai\Actions\DeliverAiAnswerToUser`: `telegram → SendTelegramSimpleQueryJob`, `vk → SendVkSimpleMessageJob`, `max → SendMaxSimpleMessageJob`, `avito → case 'avito'`, `email → case 'email'` (all built-in, wired directly in `DeliverAiAnswerToUser` — none of these are examples of the platform-registry pattern). The editing/deletion of the AI bot draft inside the supergroup topic (Accept callback) stays on `SendTelegramMessageJob` with the AI bot token regardless of user platform — that side is always Telegram. For any other, non-built-in platform, delivery is delegated to a `PlatformChannel` registered in `App\Platform\PlatformChannelRegistry` by a pluggable third-party module — the core needs no edits to support it. Only when no channel is registered does it log `ai_deliver_unsupported_platform` and skip delivery.
...
| GigaChat | `ai.gigachat_client_id`, `ai.gigachat_client_secret`(secret), `ai.gigachat_base_url`, `ai.gigachat_model`, `ai.gigachat_max_tokens`, `ai.gigachat_temperature`, `ai.gigachat_path_cert`, `ai.gigachat_scope` |
...
| GigaChat | `ai.gigachat_client_id`, `ai.gigachat_client_secret`, `ai.gigachat_base_url`, `ai.gigachat_model`, `ai.gigachat_path_cert`, `ai.gigachat_scope`, … | `GigaChat-2-Max` |
