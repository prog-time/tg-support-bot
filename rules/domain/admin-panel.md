...
**BR-004** — Livewire polling interval is 5 seconds. The poll target is `pollUpdates(document.hasFocus())` (`wire:poll.5s.keep-alive`), which refreshes the dialog list and — when a dialog is open — reloads the active message thread, so incoming messages appear in the centre pane without a manual refresh. It scrolls to the bottom whenever the message count grew, regardless of focus; it bumps `manager_last_read_at` only while `$hasFocus` is true. The focus gate exists because `.keep-alive` keeps `pollUpdates()` firing while the tab/PWA is backgrounded (for desktop/push notifications) — without it, a dialog left open before backgrounding would silently mark every new message read while nobody was looking at it. New messages loaded while unfocused set `$pendingReadMark = true`; the first tick afterwards where `$hasFocus` is true marks the dialog read even if that tick itself has no new messages, so returning focus doesn't leave it stuck unread. `loadMessages()` is a pure loader; callers (`selectChat`, `sendReply`, `pollUpdates`) emit the `messages-updated` browser event when a scroll is wanted. Do not change the interval without load analysis — each open browser tab generates DB queries every 5 seconds.
...
**BR-014e** — The «Подключить Telegram» screen also has an **optional** «IP-адрес сервера» field (`telegram.webhook_ip_address`, plain text — not a secret, not masked). When non-blank it must be a valid IP (`filter_var(..., FILTER_VALIDATE_IP)`); `validateFields()` rejects an invalid value with a per-field error, same as the required fields. Unlike the secret fields it is **always persisted as entered** in `saveTelegram()` — including blank, so clearing the field removes the stored value (no blank-submission guard, since there is nothing to protect from overwriting: this value isn't a secret pre-filled with a masked placeholder). When set, `WebhookRegistrationService::registerTelegram()` includes it as `ip_address` in the `setWebhook` call, so Telegram connects to that fixed IP directly instead of re-resolving `APP_URL`'s domain via DNS on every update; when blank, `ip_address` is omitted entirely and Telegram falls back to its normal DNS resolution.
_Enforced in:_ `IntegrationChannelPage::validateFields()` (telegram branch, IP format check); `IntegrationChannelPage::saveTelegram()`; `WebhookRegistrationService::registerTelegram()`; `app/Services/Settings/SettingKeyRegistry.php @ telegram.webhook_ip_address`; `resources/views/livewire/settings/integration-channel-page.blade.php`
...
The «Уведомления в браузере» toggle also drives Web Push (see «Web Push (background delivery)» below) — accepting the browser permission additionally subscribes the device for push, so background/foreground desktop notifications and locked-screen push share one on/off switch from the operator's point of view.

`showNotification()` displays the notification via `navigator.serviceWorker.ready.then(reg => reg.showNotification(...))` — **not** the bare `new Notification()` constructor. This is required for iOS: Safari (16.4+, including an installed home-screen PWA) only supports notifications routed through an active service worker; a page-level `new Notification()` call is silently unsupported there, so notifications would never appear on iPhone regardless of permission state. The click-to-focus behavior that used to live on the `Notification` instance's `onclick` now lives in the SW's `notificationclick` listener (focuses an open `/admin/` client, or opens `/admin/chats` if none is open).
...
SW strategy: HTML **navigations** are network-first with the precached `public/offline.html` shell as fallback — **authenticated HTML is never written to the cache** (security); static assets (`/build/`, `/icons/`, manifest) are cache-first; Livewire/AJAX/POST and cross-origin requests pass straight through (so 5 s polling, Web Notifications, Web Audio and the favicon badge are unaffected online). A `notificationclick` listener focuses an existing `/admin/` client (or opens `/admin/chats`) when the operator taps a notification — see the Notifications & sound card note above for why notifications are routed through the SW at all. Registration lives in `resources/js/app.js` and runs only on `/admin/*` pages in a secure context (HTTPS/localhost). Both admin layouts (`admin-chat`, `admin-settings`) carry the `<link rel="manifest">`, `theme-color`, and apple-touch-icon `<head>` tags. Install uses the browser's native prompt (no custom in-app button). Deploy note: `resources/js/app.js` is bundled, so an asset rebuild (`npm run build`) is required for the SW registration to ship. Tests: `tests/Feature/Admin/PwaTest.php`.

### Web Push (background delivery)

The poll-driven notifications above only fire while the PWA is open or recently backgrounded — iOS (and mobile browsers generally) suspend JS execution once the app is fully backgrounded or the screen locks, so nothing can notify the operator in that state without a real push channel. Web Push (VAPID, delivered through the browser's push service — APNs on iOS 16.4+) closes that gap and reaches an installed PWA even when it is closed or the device is locked.

- **Setup**: `php artisan webpush:generate-vapid-keys` (`App\Console\Commands\GenerateVapidKeys`) generates the VAPID keypair once via `Minishlink\WebPush\VAPID::createVapidKeys()` and stores it through `SettingsService` — `webpush.vapid_public_key`, `webpush.vapid_private_key` (secret), `webpush.vapid_subject` (the app URL). No settings screen — same "auto-captured, not manually entered" pattern as `avito.user_id`/`telegram_ai.id`. Refuses to overwrite an existing keypair unless `--force` is passed (regenerating invalidates every existing browser subscription).
- **Subscribing**: after `Notification.requestPermission()` resolves to `granted`, the Alpine `enableNotifications()` handler in `general-settings-page.blade.php` also calls `subscribePush()`, which subscribes via `navigator.serviceWorker.ready.then(reg => reg.pushManager.subscribe({applicationServerKey: ...}))` (VAPID public key passed from `GeneralSettingsPage::$vapidPublicKey`, converted with the `window.urlBase64ToUint8Array()` helper in `resources/js/app.js`) and posts the resulting subscription to `GeneralSettingsPage::savePushSubscription()`, which upserts a `PushSubscription` row keyed by `endpoint` under `Auth::id()`. Best-effort: silently skipped if the SW or VAPID key aren't ready.
- **Sending**: `App\Observers\MessageObserver::created()` — bound in `AppServiceProvider::boot()` via `Message::observe(MessageObserver::class)` — dispatches `App\Jobs\SendWebPushNotificationJob` for every `Message` row with `message_type=incoming`. This is the codebase's first Eloquent Observer: incoming messages are written from ~10 separate platform-specific places (Telegram, VK, Max, Avito, Email, External sources), and an Observer on the shared `Message` model is the one point that covers all of them without a dispatch call duplicated into each platform service — everywhere else in the codebase still favors explicit Action/Service calls over model events. The notification title is `"{Platform} · {sender name}"` (`SendWebPushNotificationJob::notificationTitle()`) — platform label uses the same `telegram→Telegram / vk→VK / max→Max / default→ucfirst()` mapping as the chat workspace header, and the name falls back `display_name ?? username ?? chat_id`; falls back to the generic `'Новое сообщение'` only if the message's `BotUser` is somehow missing. The body is the message text verbatim (not truncated) — mobile OSes truncate the banner visually on their own.
- **Delivery**: `SendWebPushNotificationJob` loops every `PushSubscription` and calls `App\Services\WebPush\WebPushSenderInterface::send()` (bound to `MinishlinkWebPushSender`, wrapping `minishlink/web-push` — the vendor client makes its own cURL calls outside Laravel's `Http` facade, so it is wrapped behind an interface for testability, mirroring `App\Modules\Ai\Contracts\AiProviderInterface`). A subscription is deleted when the push service reports it expired/gone; a non-expired failure is logged and the row is kept. `MinishlinkWebPushSender` constructs `Minishlink\WebPush\WebPush` with an explicit PSR-3 `logger` (`Log::channel('app')`) — without it, the vendor client reports a missing GMP/BCMath extension via `trigger_error(E_USER_NOTICE)`, which Laravel's error handler converts into a thrown `ErrorException` and aborts the send before it ever reaches the push service; passing a logger makes the library log that notice instead of raising it. The whole `new WebPush(...)` + `sendOneNotification()` call is wrapped in a `try/catch (\Throwable)` so any other unexpected failure (e.g. a network error) degrades to a logged failed `WebPushSendResult` instead of bubbling into the webhook request that saved the message.
- **Receiving**: the SW's `push` listener (`PwaController::serviceWorkerScript()`) parses the JSON payload and calls `self.registration.showNotification(data.title, {body: data.body, tag: 'tg-support-chat', renotify: true})` — reuses the same `notificationclick` handler as the foreground path.
- **Schema**: `push_subscriptions` — `user_id` (FK → `users`, cascade delete), `endpoint` (unique), `public_key` (p256dh), `auth_token` (auth). See `database/schema.md`.
